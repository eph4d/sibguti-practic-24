...
/**
 * Контроллер для добавления email и верификации через OTP
 */
@RestController
...
public class AddEmailController {
     ...
     */
    @PostMapping("/send")
    public ResponseEntity<Object> addOtpSend(@RequestBody @Valid EmailModel model, BindingResult bindingResult) {
    ...
    /**
     * Подтверждает OTP код и добавляет email в аккаунт пользователя
     *
     ...
     */
    @PostMapping("/confirm")
    public ResponseEntity<Object> addOtpConfirm(@RequestBody @Valid EmailAndCodeModel model, BindingResult bindingResult) {
        if (bindingResult.hasErrors()) {
